    ...
    <el-form :model="form">
      ...
      <el-form-item label="评价分数" :label-width="formLabelWidth">
        <div class="demo-rate-block">
          <el-rate v-model="form.rate" disabled ></el-rate>
        </div>
      </el-form-item>
      <el-form-item label="评价详情" :label-width="formLabelWidth">
        <el-text> {{form.formDetail}} </el-text>
      </el-form-item>
    </el-form>
    <template #footer>
      <span class="dialog-footer">
        <el-button @click="dialogRatingFormVisible = false">取消</el-button>
        <el-button type="primary" @click="dialogRatingFormVisible = false"  >
          确认
        </el-button>
      </span>
    </template>
...
export default {
  name: "MyRated",
  mounted() {
    axios.get('cart/findRated').then(res=>{
...
// 将年、月、日、小时、分钟、秒拼接为字符串，可以根据需要自定义分隔符和格式
  ...
  setup(){
    ...
    const openPaymentDialog=(item)=>{
      ...
      form.rate=item.starRating
      form.formDetail=item.content
